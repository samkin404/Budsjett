...
var DF=[
  {id:'bolig',name:'Boligavdrag',amount:20000,note:'Månedlig avdrag',icon:'🏠',color:'#1e3a5f'},
  {id:'bil',name:'Bilforsikring',amount:855,note:'Gjensidige',icon:'🚗',color:'#1e3a3a'},
  {id:'mobil',name:'Mobilabonnement',amount:328,note:'Fast pris',icon:'📱',color:'#1e2a3a'},
  {id:'strom',name:'Strøm',amount:1287,note:'Variabelt snitt',icon:'⚡',color:'#3a2e10'},
  {id:'inet',name:'Internett',amount:799,note:'Fast abonnement',icon:'🌐',color:'#1e2a3a'},
  {id:'netflix',name:'Netflix',amount:189,note:'Strømmetjeneste',icon:'🎬',color:'#3a1a1a'},
  {id:'viaplay',name:'Viaplay',amount:699,note:'Strømmetjeneste',icon:'📺',color:'#2a1a3a'},
  {id:'gym',name:'Treningssenter',amount:402,note:'Fast abonnement',icon:'💪',color:'#1a3a1e'},
  {id:'golf',name:'Golf',amount:402,note:'Fast abonnement',icon:'⛳',color:'#1a3a1e'},
  {id:'leasing',name:'Elbil leasing',amount:5500,note:'Valgt modell',icon:'🔋',color:'#1a2e3a'}
];
var DV=[
  {id:'mat',name:'Mat og dagligvarer',amount:5200,icon:'🛒',color:'#2a1e10'},
  {id:'lading',name:'Lading elbil',amount:300,icon:'⚡',color:'#3a2e10'},
  {id:'klar',name:'Klær og sko',amount:1000,icon:'👟',color:'#2a1a3a'},
  {id:'rest',name:'Restauranter / kafe',amount:1500,icon:'🍽️',color:'#3a1a1a'},
  {id:'fritid',name:'Fritid og hobby',amount:1500,icon:'🎯',color:'#1a3a1e'},
  {id:'div',name:'Diverse / uforutsett',amount:1000,icon:'🎲',color:'#2a2a2a'}
];

var cy=new Date().getFullYear(),cm=new Date().getMonth();
var mode='budget',obStep=0,addTo=null,txItem=null,emTarget=null,chartObj=null;
...
function gF(){return ls('fx',JSON.parse(JSON.stringify(DF)))}
function gV(){return ls('vr',JSON.parse(JSON.stringify(DV)))}
    ...
    try{
      ...
      if(r.error){
        ...
        setTimeout(function(){
          ...
          e.textContent='Bekreftelseslenken er utløpt. Registrer deg på nytt.';
...
async function doLogin(){
  ...
  try{
    ...
    if(r.error){
      ...
      btn.textContent='Logg inn';btn.disabled=false;return;
    }
    ...
    err.textContent='Noe gikk galt, prøv igjen';err.style.display='block';
    btn.textContent='Logg inn';btn.disabled=false;
  }
}
...
async function doRegister(){
  ...
  try{
    var r=await supa.auth.signUp({
      ...
      options:{emailRedirectTo:'https://samkin404.github.io/Budsjett/'}
    });
    ...
    ok.textContent='✓ Sjekk e-posten din (også søppelpost) for bekreftelseslenke!';
    ok.style.display='block';btn.textContent='Opprett konto';btn.disabled=false;
  }catch(e){
    err.textContent='Noe gikk galt, prøv igjen';err.style.display='block';
...
function obNext(s){
  ...
  obStep=s;
  document.getElementById('ob'+s).classList.add('active');
  document.getElementById('dot'+s).classList.add('active');
  if(s===1)setTimeout(function(){document.getElementById('ob-income').focus()},300);
}
function finishOb(){
  var inc=parseInt(document.getElementById('ob-income').value)||56000;
  var d=gMD(cy,cm);d.income=inc;d.set=true;sMD(cy,cm,d);
  lss('ob',true);
  document.getElementById('view-ob').style.display='none';
  document.getElementById('view-main').style.display='block';
  upd();
...
function setMode(m){
  ...
  document.getElementById('mode-hint').textContent=m==='budget'
    ...
    :'Beløp er låst · Trykk kategori for å logge';
  rFixed();rVar();updSum();
}
...
function updInc(){
  ...
  document.getElementById('ic-month-label').textContent='Inntekt – '+M[m]+' '+y;
  document.getElementById('ic-val').textContent=fmt(base);
  document.getElementById('ic-total').textContent=fmt(tot);
...
function delExt(i){var e=gExt(cy,cm);e.splice(i,1);sExt(cy,cm,e);updInc();updSum()}
function startIncEdit(){
  document.getElementById('ic-val').style.display='none';
  var inp=document.getElementById('ic-inp');
  inp.style.display='block';inp.value=gInc(cy,cm);inp.focus();inp.select();
}
function commitIncEdit(){
  var v=parseInt(document.getElementById('ic-inp').value)||56000;
  var d=gMD(cy,cm);d.income=v;d.set=true;sMD(cy,cm,d);
  document.getElementById('ic-inp').style.display='none';
  document.getElementById('ic-val').style.display='block';
  upd();
}

// ── SUMMARY ──
function updSum(){
  var y=cy,m=cm,inc=gTI(y,m),fx=gF(),vr=gV();
  ...
  var sv=Math.round(inc*0.10),lf=inc-ft-vt-sv;
  document.getElementById('sum-fixed').textContent=fmt(ft);
  document.getElementById('sum-var').textContent=fmt(vt);
  document.getElementById('sum-save').textContent=fmt(sv);
  document.getElementById('fixed-total').textContent=fmt(ft);
  document.getElementById('var-total').textContent=fmt(vt);
  var el=document.getElementById('sum-left');
  el.textContent=fmt(lf);el.className='s-val '+(lf>=0?'green':'red');
}

// ── FIXED LIST ──
function rFixed(){
  var items=gF(),el=document.getElementById('fixed-list');
  if(!items.length){el.innerHTML='<div class="empty-state">Ingen kategorier</div>';return}
...
function rVar(){
  ...
  if(!items.length){el.innerHTML='<div class="empty-state">Ingen kategorier</div>';return}
  var y=cy,m=cm,locked=mode==='actual';
  el.innerHTML=items.map(function(item,i){
    ...
    return '<div class="expense-row var-row" onclick="openTx('+i+')">'+
      ...
      '<div class="amt-col" onclick="event.stopPropagation()">'+
        (locked?'':'<input class="exp-amt-inp" id="vi'+i+'" type="number" inputmode="numeric" value="'+item.amount+'" onblur="cV('+i+')" onkeydown="if(event.key===\'Enter\')this.blur()">')+
        '<span class="exp-amt'+(locked?' locked':'')+'" id="va'+i+'" '+(locked?'':'onclick="event.stopPropagation();sV2('+i+')"')+'>'+fmt(item.amount)+'</span>'+
      '</div>'+
...
function commitTx(){
  if(!txItem)return;
  var v=parseFloat(document.getElementById('tx-amt-inp').value)||0;
  if(!v)return;
  var y=cy,m=cm,id=txItem.item.id,txs=gTxs(y,m,id);
...
function dTx(i){
  if(!txItem)return;
  var y=cy,m=cm,id=txItem.item.id,txs=gTxs(y,m,id);
  txs.splice(i,1);sTxs(y,m,id,txs);rTxList();rVar();updSum();
}
function closeTx(){
...
function commitAdd(){
  ...
  if(!name)return;
  if(addTo==='income'){
    var e=gExt(cy,cm);e.push({id:uid(),name:name,amount:amount});sExt(cy,cm,e);
    closeSheet();updInc();updSum();
  }else if(addTo==='fixed'){
...
function rGraf(){
  ...
  chartObj=new Chart(cv,{
    type:'line',
    data:{
      labels:e.map(function(x){return M[x.month].slice(0,3)+' '+String(x.year).slice(2)}),
      datasets:[{data:e.map(function(x){return x.left}),borderColor:'#4ade80',backgroundColor:'rgba(74,222,128,0.08)',borderWidth:2,pointBackgroundColor:'#4ade80',pointRadius:4,tension:0.3,fill:true}]
    },
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return fmt(c.raw)}},backgroundColor:'#1a2236',titleColor:'#f1f5f9',bodyColor:'#4ade80',borderColor:'rgba(255,255,255,0.1)',borderWidth:1}},scales:{x:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#64748b',font:{size:10}}},y:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#64748b',font:{size:10},callback:function(v){return Math.round(v/1000)+'k'}}}}}
  });
  document.getElementById('graf-history').innerHTML='<div class="section" style="margin-top:14px"><div class="hist-card">'+
    e.slice().reverse().map(function(x){var c=x.left>=0?'#4ade80':'#f87171';return '<div class="hist-row"><div><div class="hist-month">'+M[x.month]+' '+x.year+'</div><div class="hist-sub">Inntekt '+fmt(x.income)+' · Spart '+fmt(x.sv)+'</div></div><div class="hist-left" style="color:'+c+'">'+(x.left>=0?'+':'')+fmt(x.left)+'</div></div>'}).join('')+
  '</div></div>';
}
function rHist(){
  ...
  el.innerHTML='<div class="hist-card">'+e.map(function(x){
    var c=x.left>=0?'#4ade80':'#f87171';
    return '<div class="hist-row"><div><div class="hist-month">'+M[x.month]+' '+x.year+'</div><div class="hist-sub">Inntekt: '+fmt(x.income)+' · Spart: '+fmt(x.sv)+'</div></div><div class="hist-right"><div class="hist-left" style="color:'+c+'">'+(x.left>=0?'+':'')+fmt(x.left)+'</div><button class="hist-del" onclick="dHist(\''+x.key+'\')">🗑</button></div></div>';
  }).join('')+'</div>';
}
function dHist(k){
